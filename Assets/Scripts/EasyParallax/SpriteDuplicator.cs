 ...
 * Creates copies of this object and arranges them seamlessly, so that one is right next to the other.
  翻訳：このオブジェクトのコピーを作成し、それらをシームレスに配置して、隣り合わせになるようにします。
 */
    [RequireComponent(
        typeof(SpriteRenderer))] //Make sure we have a SpriteRenderer, because we need the width of the sprite 翻訳：SpriteRendererがあることを確認してください。スプライトの幅が必要なので
    public class SpriteDuplicator : MonoBehaviour
    {
        /**
     * The number of copies of the object that we will have at any given time, including this object 翻訳：このオブジェクトを含む、いつでも持っているオブジェクトのコピーの数
     */
